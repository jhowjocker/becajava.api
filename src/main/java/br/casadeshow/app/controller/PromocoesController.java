...
public class PromocoesController {
		...
		public PromocoesService _service;
		
		@Autowired
		public PromocoesController(PromocoesService service) {
			_service = service;
		}

		@PostMapping
		public ResponseEntity inserir(@RequestBody Promocoes promocoes) {
			_service.inserir(promocoes);
			return ResponseEntity.status(HttpStatus.CREATED).body("Banda Inserida com sucesso!");
		}
		
		@GetMapping
		public ResponseEntity listar() {
			Iterable<Promocoes> promocoes = _service.listar();
		return ResponseEntity.status(HttpStatus.OK).body(promocoes);
		}
